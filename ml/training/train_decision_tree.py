"""
PhishGuard AI - Baseline Model 2: Decision Tree.
Trains CART Decision Tree Classifier on training partition, evaluates on test partition.
"""
...
MODEL_SAVE_PATH = MODELS_DIR / "decision_tree.joblib"


def train_decision_tree(pipeline: FeaturePipeline, train_df: pd.DataFrame, test_df: pd.DataFrame) -> dict:
    ...
    plot_and_save_confusion_matrix(metrics["confusion_matrix"], "Decision Tree")
    print(f"[+] Decision Tree trained in {training_time:.2f}s | Test Accuracy: {metrics['accuracy']} | F1: {metrics['f1']}")
