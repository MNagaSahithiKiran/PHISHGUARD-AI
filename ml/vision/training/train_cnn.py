"""
PhishGuard AI - Baseline Vision CNN Training Pipeline
Trains the PhishVisionCNN model with deterministic random seeds, early stopping,
and validation checkpointing. Saves trained weights to models_artifacts.
"""
...
def train_baseline_cnn():
    ...
    # 3. Model & Optimizer
    cnn_cfg = config.get("baseline_cnn", {})
    model = PhishVisionCNN(num_classes=1, dropout_rate=cnn_cfg.get("dropout_rate", 0.4))
    model.to(device)

    criterion = nn.BCEWithLogitsLoss()
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=float(cnn_cfg.get("learning_rate", 0.001)),
    ...
    best_model_path = ARTIFACTS_DIR / "baseline_cnn_best.pt"
    ...
    print(f"\n[Baseline CNN] Training complete in {total_time}s. Best Epoch: {best_epoch} (Val F1: {best_val_f1:.4f})")
    print(f"[Artifact] Saved best weights to: {best_model_path}")

    # Save history
    history_path = EXPERIMENTS_DIR / "history_baseline_cnn.json"
